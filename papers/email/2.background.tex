...
Section~\ref{sec:measure_forwarding_mechs_and_arc}.  Finally, I
briefly survey related work on email security, particularly
those whose insights I have built upon.
% In the remainder of the paper, I explore email forwarding in more depth and show how the inherent conflict between the functional goals of forwarding and anti-spoofing mechanisms enable a wide range of spoofing attacks.
% I then describe the range of forwarding mechanisms in use and how
...
%  Figure~\ref{fig:mech_smtp_headers}, but I consider them
...
% is not needed since I do not ever discuss it later.}
...
%% \grant{I think I can completely cut this paragraph if I need space (IV.D.2 gives enough context about UI defenses without this.)}
...
% email spoofing.
%Next, I consider email forwarding and how it complicates these
...
For simplicity I show a single forwarder in our example, but email can pass through multiple forwarders in common use cases. % (note, while the figure shows a single forwarder, it is possible to have multiple such forwarders in series).
...
% \emph{forwarding mechanisms} --- to forward email.  I describe four
...
% forwarding mechanisms described above, for which I can find some
...
% %  kind?}  As I previously discussed, email authentication using SPF,
...
% %modifying it and sending it along.  However, as I demonstrate
